...
!                  State_Chm%SpcData(id_DST1)%Info%MW_g / AVO
       ...
       IF ( IS_FULLCHEM .and. id_HMS > 0 ) THEN
          ...
          K_CLD(5) = KaqHMS  * FC
          ! SDE: this approach could very occasionally cause the integrator to blow
          ! up. Experimentally changing it from
          !      HMS + OH -> 2SO4 + CH2O - SO2
          ! to
          !      HMS + OH + SO2 -> 2SO4 + CH2O
          ! and dividing the rate by C(SO2) at t=0 to compensate (imperfectly!)
          ! SDE: this approach could very occasionally cause the integrator to blow
          ! up. Experimentally changing it from
          !      HMS + OH -> 2SO4 + CH2O - SO2
          ! to
          ...
          ! and dividing the rate by C(SO2) at t=0 to compensate (imperfectly!)
          !K_CLD(6) = KaqHMS2 * FC
          If (Spc(id_SO2)%Conc(I,J,L) .lt. 1.0_dp ) Then
              K_CLD(6) = 0.0 ! Disable reaction if too little SO2
          Else
